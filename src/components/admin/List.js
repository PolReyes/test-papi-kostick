...
const List = (props) => {
    ...
    async function getData() {
        let result = await fetch(`http://159.223.170.205/dev/test/user/` + props.match.params.id)
    ...
    async function getCandidato() {
        let result = await fetch(`http://159.223.170.205/dev/user/` + props.match.params.id)
        result = await result.json();
        if (result.code === 1) {
    ...
    async function editFechaVen() {
        ...
        let result = await fetch(`http://159.223.170.205/dev/test/days`, {
            method: 'PUT',
            headers: {
