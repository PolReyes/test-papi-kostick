...
const Usuarios = () => {
    ...
    async function enviarEmail(id){
        let result= await fetch(`http://137.184.89.132/dev/user/sendemail/`+id)
        result = await result.json();
        if(result.code===1){
            setAlert(result.message)
            setError("")
    ...
    async function estadoTest(estado,id){
        if (estado==='Vencido') {
            ...
            let result = await fetch(`http://137.184.89.132/dev/user/password`,{
                method:'POST',
                headers:{
                    "Content-Type":"application/json",
                    "Accept":"application/json"
    ...
    async function getListTest(id){
        handleOpen();
        let result= await fetch(`http://137.184.89.132/dev/test/user/`+id)
        result = await result.json();
        setList(result.data)
        //console.log(data)
